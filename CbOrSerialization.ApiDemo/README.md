...
A minimal API demo showcasing CBOR serialization in ASP.NET Core using the CbOrSerialization library.
...
dotnet run --project CbOrSerialization.ApiDemo
...
```csharp
// Serialize to CBOR
var cborData = CbOrSerializer.Serialize(users, cborContext.ListOfSimpleUser);
return Results.Bytes(cborData, "application/cbor");

// Deserialize from CBOR
var user = CbOrSerializer.Deserialize(cborData, cborContext.SimpleUser);
```

## Project Structure

```
CbOrSerialization.ApiDemo/
├── Models.cs              # User model and CBOR context
├── Program.cs             # Minimal API configuration and endpoints
├── README.md              # This documentation
└── CbOrSerialization.ApiDemo.csproj
```
