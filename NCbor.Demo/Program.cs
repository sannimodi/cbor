...
var person = new Person
{
    ...
    InternalNotes = "This should not be serialized due to [NCborIgnore]",
    Status = "active"
};

// Serialize enhanced person
var personCbor = NCborSerializer.Serialize(person, MyContext.Default.Person);
...
var deserializedPerson = NCborSerializer.Deserialize(personCbor, MyContext.Default.Person);
Console.WriteLine($"✅ Deserialized Person:");
Console.WriteLine($"   ID: {deserializedPerson.PersonId}");
Console.WriteLine($"   Name: {deserializedPerson.Name} (using [NCborPropertyName])");
...
Console.WriteLine($"   Internal Notes: '{deserializedPerson.InternalNotes}' (should be empty due to [NCborIgnore])");
Console.WriteLine();
...
var prefsCbor = NCborSerializer.Serialize(userPreferences, MyContext.Default.DictionaryOfStringAndString);
var deserializedPrefs = NCborSerializer.Deserialize(prefsCbor, MyContext.Default.DictionaryOfStringAndString);
...
var scoresCbor = NCborSerializer.Serialize(teamScores, MyContext.Default.DictionaryOfStringAndInt32);
var deserializedScores = NCborSerializer.Deserialize(scoresCbor, MyContext.Default.DictionaryOfStringAndInt32);
...
var deptCbor = NCborSerializer.Serialize(engineeringDept, MyContext.Default.Department);
var deserializedDept = NCborSerializer.Deserialize(deptCbor, MyContext.Default.Department);
...
var teamCbor = NCborSerializer.Serialize(team, MyContext.Default.ListOfPerson);
var deserializedTeam = NCborSerializer.Deserialize(teamCbor, MyContext.Default.ListOfPerson);
...
var nullsCbor = NCborSerializer.Serialize(personWithNulls, MyContext.Default.Person);
var deserializedNulls = NCborSerializer.Deserialize(nullsCbor, MyContext.Default.Person);
