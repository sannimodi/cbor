...
using CbOrSerialization.ApiDemo;
...
app.MapGet("/api/users/{id:guid}", (Guid id) =>
{
    ...
    var cborData = CbOrSerializer.Serialize(user, cborContext.SimpleUser);
    return Results.Bytes(cborData, "application/cbor");
})
...
app.MapPost("/api/users/cbor", async (HttpRequest request) =>
{
    try
    {
        ...
        var responseCbor = CbOrSerializer.Serialize(newUser, cborContext.SimpleUser);
