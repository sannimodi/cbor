...
public class CbOrGuidTests
{
    ...
    public void Serialize_GuidModel_WithValidGuid_SerializesCorrectly()
    {
        ...
        // Act
        var serialized = CbOrSerializer.Serialize(model, _context.GuidModel);
    ...
    public void Serialize_Then_Deserialize_GuidModel_RoundTripSucceeds()
    {
        ...
        // Act
        var serialized = CbOrSerializer.Serialize(original, _context.GuidModel);
        var deserialized = CbOrSerializer.Deserialize(serialized, _context.GuidModel);
    ...
    public void Serialize_MultipleGuids_EachHasUniqueRepresentation()
    {
        ...
        // Act
        var serialized1 = CbOrSerializer.Serialize(model1, _context.GuidModel);
        var serialized2 = CbOrSerializer.Serialize(model2, _context.GuidModel);

        // Assert
        serialized1.Should().NotEqual(serialized2);
        
        var deserialized1 = CbOrSerializer.Deserialize(serialized1, _context.GuidModel);
        var deserialized2 = CbOrSerializer.Deserialize(serialized2, _context.GuidModel);
    ...
    [Fact]
    public void Deserialize_CorruptedGuidData_ThrowsCbOrDeserializationException()
    {
        // Arrange - Create valid serialized data first
        var model = new GuidModel { Id = Guid.NewGuid(), Name = "Test", OptionalId = null };
        var validSerialized = CbOrSerializer.Serialize(model, _context.GuidModel);
        
        // Corrupt the data by truncating it
        var corruptedData = validSerialized.Take(validSerialized.Length / 2).ToArray();

        // Act & Assert
        var act = () => CbOrSerializer.Deserialize(corruptedData, _context.GuidModel);
